...
- Host-run Python commands use `localhost` defaults from `.env`; containerized services use Compose service names (`sqlserver`, `seaweedfs`, `elasticsearch`) on the `de` network (`airflow/docker-compose.yml`).
- Airflow is intentionally a separate Compose project that joins external network `de-playground_de`; if repo folder name changes, update that network name (`airflow/docker-compose.yml`).
