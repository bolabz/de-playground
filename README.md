...
> or `:9200` (Elasticsearch) almost always means that service just isn't running.
...
| 5b | Airflow 3 via official Helm chart on k3d (absorbs the EOL fix) | ⬜ next |
...
## Orchestrate: Airflow (Phase 4)

Airflow is the **control plane** — it schedules and triggers the pipeline (extract → transform
→ index) with retries and backfills. It runs from its own compose file and joins the main
stack's `de` network, so its tasks reach SQL Server, SeaweedFS, and Elasticsearch by name.

```sh
make up-serve     # the stack Airflow orchestrates must exist first (creates the `de` network)
make up-airflow   # build + start Airflow (first build is slow: JDK + ODBC + pipeline venv)
```

Open http://localhost:8081 (login **admin / admin**), unpause the `wwi_pipeline` DAG, and
trigger it — or let it run on its `@daily` schedule. Tasks: `extract_bronze` →
`transform_silver_gold` → `index_elasticsearch`, each with 2 retries.

Backfill a date range (re-runs the pipeline per scheduled interval):

```sh
docker compose -f airflow/docker-compose.yml exec airflow-scheduler \
  airflow dags backfill -s 2026-01-01 -e 2026-01-03 wwi_pipeline
```
...
- Each task is a `BashOperator` calling the same `python -m de_playground.*` entrypoints you
  run by hand, via an **isolated pipeline venv** baked into the Airflow image (Airflow 2.x
  needs SQLAlchemy <2.0; the pipeline needs 2.0 — separate venvs avoid the clash).
- Transform runs Spark in **local mode** inside the worker (the image has a JDK); it does not
  use the standalone Spark cluster.

> Honest caveat: in real enterprise Airflow / Azure Data Factory, the control plane *triggers
> external compute* (e.g. Databricks) rather than running it in the worker. Running it in-worker
> here keeps the rig to one image and no Docker socket — simpler to run solo. The orchestration
> concepts (DAGs, dependencies, retries, scheduling, backfills) transfer; the execution topology
> is the part you'd do differently in the cloud.
>
> The external network is assumed to be `de-playground_de` (Compose names it `<folder>_de`).
> If your repo folder isn't `de-playground`, find the name via `docker network ls` and update
...
make platform-up       # k3d cluster: 1 server + 2 agents (compose stack unaffected)
make api-image         # build the API image + import it into the cluster
...
(`host.k3d.internal`) — the hybrid is deliberate while the data services stay in compose.
...
> Next increment (5b): Airflow 3 via its official Helm chart with KubernetesExecutor —
> retiring the EOL Airflow 2.x compose setup (BACKLOG P1).

## Layout

`src/de_playground/` is the importable Python package (extract / transform / load /
common / config); `dags/`, `api/`, `spark/`, `airflow/`, `sql/`, `tests/`, `notebooks/`,
