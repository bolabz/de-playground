...
- **Airflow 2.11 → 3.x** — 🔥 **42 days past EOL** (EOL 2026-04-22). CLI / compose / auth
  changed in 3.x — revisit `airflow/` image, init, and the OTel commented block. SQLAlchemy
  `<2.0` pin is lifted in 3.x (simplifies our two-venv workaround).
...
- **IaC + cloud deploy target** — ⏳ *in progress via the Phase 5 platform track* (2026-06-03):
  ...
  - **5b — Airflow 3 via official Helm chart on k3d** (KubernetesExecutor; absorbs the P1
    Airflow EOL fix and retires the compose Airflow).
  - **5c — k3d-managed registry instead of `k3d image import`** (real push/pull), then CI
    builds the image + bumps the chart tag = full CD loop.
...
- **Airflow OTel metrics** — flip on the commented block in `airflow/docker-compose.yml` (needs the
  otel exporter in the Airflow image).
